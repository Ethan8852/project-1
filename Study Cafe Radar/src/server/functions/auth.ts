...
async function getServerDeps() {
  const [{ getDB, getEnv }, { hashPassword, verifyPassword, signJWT, verifyJWT }, { getSessionToken, setSessionCookie, clearSessionCookie }] =
    await Promise.all([
      import("../context"),
      import("../crypto"),
      import("../session"),
    ]);
  return { getDB, getEnv, hashPassword, verifyPassword, signJWT, verifyJWT, getSessionToken, setSessionCookie, clearSessionCookie };
}

export async function requireUserId(): Promise<string> {
  const { getEnv, verifyJWT, getSessionToken } = await getServerDeps();
...
export const register = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    const { getDB, getEnv, hashPassword, signJWT, setSessionCookie } = await getServerDeps();
    const db = getDB();
    const env = getEnv();
...
export const login = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    const { getDB, getEnv, verifyPassword, signJWT, setSessionCookie } = await getServerDeps();
    const db = getDB();
    const env = getEnv();
...
export const logout = createServerFn({ method: "POST" }).handler(async () => {
  const { clearSessionCookie } = await getServerDeps();
  clearSessionCookie();
  return { ok: true };
});
...
export const getMe = createServerFn({ method: "GET" }).handler(async () => {
  const { getDB, getEnv, verifyJWT, getSessionToken } = await getServerDeps();
  const token = getSessionToken();
